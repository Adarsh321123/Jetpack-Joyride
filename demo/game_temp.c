...
const vector_t RESET_POS = {50, 70};
...
// lets do this
// make a timetracker var in the struct
// each time a laser is added, record the time of last creation
// when removing, check if current_time - time_of_last_creation >= some amount
// now somehow add the logic for multiple at the same time

//lets implement a method for laser activation now
//random rendering for when laser warning appears
//wait for a fixed amount of time until activate
// - when not activate, make sure collision doesnt affect
//wait for a fixed amount of time until deactivate
...
const double LASER_GENERATION_TIME = 7;
...
typedef enum { CEILING, GROUND, ZAPPER, LASER } body_type_t;
...
body_t *make_laser(vector_t center) {
  if (bool) {
    ...
    body_t *laser = body_init_with_info(laser_shape, INFINITY, white,
...
void add_walls(state_temp_t *state) {
  ...
  assert(ceiling != NULL);
  list_t *ground_shape =
      make_rectangle((vector_t){MAX.x / 2, MIN.y}, MAX.x, WALL_DIM);
...
game_play_state_t *game_play_init() {
  ...
  add_force_creators(game_play_state); // useless code


  game_play_state->curr_state = GAME_PLAY;

  game_play_state->state = state;
  game_play_state->time_zapper = 0;
...
void add_zapper(game_play_state_t *game_play_state, double dt) {
  game_play_state->time_zapper += dt;
  if (game_play_state->time_zapper >= ZAPPER_GENERATION_TIME) {
...
void add_laser(game_play_state_t *game_play_state, double dt) {
  ...
  if (game_play_state->time_laser >= LASER_GENERATION_TIME) {
    fprintf(stderr, "added laser!\n");
